...
% 1.参考论文Ant System:Optimization by a colony of cooperating agents完成下列代码
% 2.重点章节 II THE THE ANT SYSTEM  和 IV EXPERIMENTAL STUDY 1:PARAMETER SETING AND BASIC PROPERTIES
% 3.尝试改写成可以随机生成20个城市的算法
%
%% 
clear;
clc;
...
while iter <= iter_max
    % 随机产生各个蚂蚁的起点城市
      ...
      for i = 1:m
          % 逐个城市计算
          for j = 1:(n - 1)
              Delta_Tau(Table(i,j),Table(i,j+1)) =                                      ;%计算第m只蚂蚁在每段路径上留下的信息素
                                                                                         %最开始Delta_Tau是0，iter=2以后开始叠加
          end
          Delta_Tau(Table(i,n),Table(i,1)) =                                            ;% 加上最后一个城市到第一个城市的回路
		  
      end
      Tau = (1-vol) * Tau + Delta_Tau;
   
    iter = iter + 1;      % 迭代次数加1 并 清空路径记录表
                    ;  %清空路径记录表
end
%--------------------------------------------------------------------------
%% 结果显示
