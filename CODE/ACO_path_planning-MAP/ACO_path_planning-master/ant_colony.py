...
class AntColony:
    ...
    def select_next_node(self, actual_node):
        ...
        for edge in actual_node.edges:
            total_sum += edge['Pheromone'] # Calculate total sum of pheromone
        ...
        for edge in actual_node.edges:  # 此时的actual_node.edges中存储的 当前点到 各个点的信息素和概率都被记录到 edges_list；概率被单独记录到p中
        ...
        return np.random.choice(edges_list,1, p)[0]['FinalNode']
                #     a = np.random.choice(edges_list, 1, p)
                #     b = a[0]['FinalNode']
                # return b
